...
export default function LoginPage() {
  ...
  const sendOTP = async () => {
    setError("");
    if (phone.length < 10) { setError("Sahi phone number dalo"); return; }
    setLoading(true);
    try {
  ...
  const verifyOTP = async () => {
    setError("");
    if (otp.length !== 6) { setError("6 digit OTP dalo"); return; }
    setLoading(true);
    try {
  ...
  const handleEmail = async () => {
    setError("");
    if (!email || !password) { setError("Email aur password dalo"); return; }
    setLoading(true);
    try {
      if (isSignup) {
    ...
    <main style={{minHeight:'100vh',background:'linear-gradient(135deg,#111 0%,#1a0505 60%,#0a1505 100%)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
      <div style={{background:'white',borderRadius:'20px',padding:'32px',width:'100%',maxWidth:'400px',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
        ...
        <div style={{display:'flex',background:'#f5f5f5',borderRadius:'12px',padding:'4px',marginBottom:'24px'}}>
          <button onClick={()=>{setTab('phone');setError('');setSuccess('');}}
            style={{flex:1,padding:'10px',border:'none',borderRadius:'10px',fontWeight:'bold',fontSize:'13px',cursor:'pointer',background:tab==='phone'?'white':'transparent',color:tab==='phone'?'#DC2626':'#888',boxShadow:tab==='phone'?'0 2px 8px rgba(0,0,0,0.1)':'none'}}>
            📱 Phone OTP
          </button>
          <button onClick={()=>{setTab('email');setError('');setSuccess('');}}
            style={{flex:1,padding:'10px',border:'none',borderRadius:'10px',fontWeight:'bold',fontSize:'13px',cursor:'pointer',background:tab==='email'?'white':'transparent',color:tab==='email'?'#DC2626':'#888',boxShadow:tab==='email'?'0 2px 8px rgba(0,0,0,0.1)':'none'}}>
            📧 Email
          </button>
        </div>
        {tab === 'phone' && (
          <div>
            {!otpSent ? (
              ...
              <>
                ...
                <input type="number" placeholder="6 digit OTP" value={otp}
                ...
                <button onClick={()=>{setOtpSent(false);setOtp('');setError('');setSuccess('');}}

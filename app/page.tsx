...
// Admin ID → ITEMS name mapping
const ADMIN_ID_MAP: Record<string, string> = {
  rohu: 'Rohu Fish', ilish: 'Ilish Hilsa', tiger: 'Tiger Prawns',
  golda: 'Golda Chingdi', kilimeen: 'Pink Perch / Kilimeen',
  sole: 'River Sole / Vaka Varal', bhetki: 'Kolkata Bhetki / Barramundi',
  blackpomfret: 'Black Pomfret / Halwa Fish', whitepomfret: 'White Pomfret / Silver Pomfret',
  surmai: 'Seer Fish / Surmai', mackerel: 'Mackerel / Bangda',
  katla: 'Katla', baasa: 'Indian Baasa', prawns: 'Prawns',
  hilsa: 'Hilsa', tilapia: 'Tilapia', rupchanda: 'Rupchanda',
  mourala: 'Mourala', singi: 'Singi Catfish', singhara: 'Singhara Catfish',
  boal: 'Boal Fish', tuna: 'Yellow Fin Tuna',
};
...
// ── SCROLLING TICKER ─────────────────────────────────────
function ScrollingTicker({ availableItems, preorderItems }: { availableItems: any[], preorderItems: any[] }) {
  const tickerItems = [
...
function SearchBar({ allItems, onSelect }: { allItems: any[], onSelect: (item: any) => void }) {
  ...
  return (
    <div ref={ref} style={{ flex: 1, minWidth: '200px', position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', background: 'white', borderRadius: '25px', padding: '10px 16px', gap: '8px', border: 'none', boxShadow: focused ? '0 0 0 2px rgba(255,255,255,0.8)' : 'none' }}>
...
export default function Home() {
  ...
  // Load items from Firebase (admin se sync)
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'items'), (snap) => {
      if (snap.exists()) {
        setAdminItems(snap.data().list || []);
      }
    });
    return () => unsub();
  }, []);
  ...
  // Admin se status merge karo ITEMS ke saath
  const getItemStatus = (itemName: string): string => {
    if (adminItems.length === 0) return 'preorder'; // default
    const found = adminItems.find(ai => ADMIN_ID_MAP[ai.id] === itemName);
    return found ? found.type : 'preorder';
  };
  const getItemPrice = (itemName: string, defaultPrice: number): number => {
    if (adminItems.length === 0) return defaultPrice;
    const found = adminItems.find(ai => ADMIN_ID_MAP[ai.id] === itemName);
    return found ? found.price : defaultPrice;
  };

  // Items jo available hain (admin ne Available mark kiya)
  const liveAvailableItems = ITEMS.filter(i => getItemStatus(i.n) === 'available');
  // Items jo preorder hain
  const livePreorderItems = ITEMS.filter(i => getItemStatus(i.n) === 'preorder');
  // Hidden items hide ho jayenge

  const allItems = [...liveAvailableItems, ...livePreorderItems];
  ...
  return (
    <main style={{fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',minHeight:'100vh',background:C.bg}}>
      ...
      {/* ── HEADER (Zappfresh style - compact) ── */}
      <header style={{background:C.red,position:'sticky',top:0,zIndex:200,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}}>
        {/* Row 1: Location + Icons */}
        <div style={{padding:'8px 14px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          {/* Left: Logo + Location */}
          <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
            <div style={{width:28,height:28,background:'white',borderRadius:7,display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>🐟</div>
            <div style={{display:'flex',alignItems:'center',gap:'4px'}}>
              <span style={{fontSize:12,color:'white',opacity:0.85}}>📍</span>
              <span style={{color:'white',fontWeight:700,fontSize:13}}>Delhi NCR</span>
              <span style={{color:'white',opacity:0.7,fontSize:11}}>▾</span>
            </div>
          </div>
          {/* Right: Icons */}
          <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
            {/* WhatsApp icon */}
            <a href="https://wa.me/918287000582" target="_blank" rel="noreferrer" style={{color:'white',textDecoration:'none',display:'flex',alignItems:'center',justifyContent:'center',width:32,height:32,background:'rgba(255,255,255,0.15)',borderRadius:8}} title="WhatsApp">
            ...
            {/* Cart icon */}
            <button onClick={()=>{setShowCart(true);setCheckoutStep('cart');}} style={{background:'rgba(255,255,255,0.15)',border:'none',borderRadius:8,width:32,height:32,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',position:'relative'}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
              {totalItems>0&&<span style={{position:'absolute',top:-4,right:-4,background:'white',color:C.red,borderRadius:'50%',width:16,height:16,display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:900}}>{totalItems}</span>}
            </button>
            {/* User icon */}
            {currentUser ? (
              <div style={{width:32,height:32,background:'white',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800,color:C.red,cursor:'pointer'}}
                onClick={()=>signOut(getAuth()).then(()=>setCurrentUser(null))}>
                {currentUser.name?currentUser.name[0].toUpperCase():'U'}
              </div>
            ) : (
              <a href="/login" style={{color:'white',textDecoration:'none',display:'flex',alignItems:'center',justifyContent:'center',width:32,height:32,background:'rgba(255,255,255,0.15)',borderRadius:8}}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              </a>
            )}
          </div>
        </div>
      ...
      <div style={{maxWidth:'1200px',margin:'0 auto',padding:'20px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
          <h2 style={{fontSize:18,fontWeight:800,color:C.text,margin:0}}>🐟 Fish</h2>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:12}}>
          {allItems.map((item,idx)=>{
            const status = getItemStatus(item.n);
            const livePrice = getItemPrice(item.n, item.p);
            ...
            return (
              ...
              >
                <div onClick={()=>{setModalItem(item);setModalPhoto(0);}} style={{position:'relative'}}>
                  ...
                  {status === 'available' ? (
                  <div style={{position:'absolute',top:8,left:8,background:'#dcfce7',color:'#16A34A',fontSize:9,fontWeight:800,padding:'2px 7px',borderRadius:10,letterSpacing:0.5}}>🟢 AVAILABLE</div>
                ) : (
                ...
                <div style={{padding:'10px 10px 12px'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    <div style={{fontWeight:900,fontSize:15,color:C.red}}>₹{livePrice}</div>
                    {cartItem ? (
